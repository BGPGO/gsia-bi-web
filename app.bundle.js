...
const PageVagas = () => {
  ...
  const empresaMatrizData = useMemo(() => {
    const map = {};
    for (const v of filteredVagas) {
      const e = v.empresa || "N/A";
      if (!map[e])
        map[e] = { fechado: 0, cancelada: 0, aberto: 0 };
      if (v.status === "FECHADO")
        map[e].fechado++;
      else if (v.status === "CANCELADA")
        map[e].cancelada++;
